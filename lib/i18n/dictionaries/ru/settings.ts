const settings = {
  // Page header
  title: "Общие настройки",
  description: "Управляйте информацией о вашем бизнесе.",

  // General business information
  generalInfoTitle: "Общая информация о бизнесе",
  generalInfoDescription: "Обновите данные, которые ваш ИИ-секретарь использует во время звонков.",
  businessName: "Название бизнеса",
  businessCategory: "Категория бизнеса",
  selectCategory: "Выберите категорию",
  businessPhone: "Рабочий номер телефона",
  email: "Адрес электронной почты",
  websiteUrl: "URL веб-сайта",
  websiteUrlPlaceholder: "https://yourbusiness.com",
  forwardingPhone: "Номер для переадресации",
  forwardingPhoneHint: "Этот номер управляется Gigahoo и не может быть изменён.",
  websiteLanguage: "Язык аккаунта",
  websiteLanguageHint: "Язык, используемый во всей панели вашего аккаунта.",

  // Business address
  addressTitle: "Адрес бизнеса",
  addressDescription: "Введите поля адреса, применимые в вашей стране.",
  addressLine1: "Адрес, строка 1",
  addressLine1Placeholder: "Улица, абонентский ящик",
  addressLine2: "Адрес, строка 2",
  addressLine2Placeholder: "Квартира, офис, помещение, здание, этаж (необязательно)",
  city: "Город / населённый пункт",
  region: "Штат / область / регион",
  selectRegion: "Выберите штат/область",
  regionPlaceholder: "Штат / область / регион",
  postalCode: "Почтовый индекс",
  country: "Страна",
  selectCountry: "Выберите страну",

  // Account security
  securityTitle: "Безопасность учётной записи",
  dangerTitle: "Опасная зона",
  dangerDescription: "Удаление аккаунта навсегда удалит ваш номер телефона, историю звонков и данные оплаты.",
  deleteAccountButton: "Удалить аккаунт",
  deleteAccountConfirm: "Аккаунт будет удалён навсегда: номер освобождён, история звонков и данные оплаты стёрты. Введите код, который мы только что отправили, чтобы подтвердить.",
  deleteVerifyTitle: "Подтвердите удаление аккаунта",
  securityDescription: "Управляйте привязанными способами аутентификации.",
  google: "Google",
  notLinked: "Не привязано",
  link: "Привязать",
  emailMethod: "Эл. почта",
  phoneNumber: "Номер телефона",
  change: "Изменить",
  linked: "Привязано",
  googleLinkFailed: "Не удалось привязать аккаунт Google",
  passwordMethod: "Пароль",
  passwordSetStatus: "Пароль установлен",
  passwordNotSet: "Пароль не установлен",
  setPassword: "Установить пароль",
  changePassword: "Изменить пароль",
  currentPassword: "Текущий пароль",
  newPassword: "Новый пароль",
  confirmPassword: "Подтвердите пароль",
  passwordPlaceholder: "Не менее 8 символов",
  passwordTooShort: "Пароль должен содержать не менее 8 символов",
  passwordMismatch: "Пароли не совпадают",
  passwordSaved: "Пароль обновлён",
  passwordSaveFailed: "Не удалось обновить пароль",
  currentPasswordRequired: "Введите текущий пароль",

  // Email / phone verification
  verifySaveEmail: "Подтвердить и сохранить эл. почту",
  verifySavePhone: "Подтвердить и сохранить телефон",
  enterEmailCode: "Введите 6-значный код, отправленный на вашу новую эл. почту.",
  enterPhoneCode: "Введите 6-значный код, отправленный на ваш новый телефон.",
  confirm: "Подтвердить",
  cancel: "Отмена",
  resendCode: "Отправить код повторно",
  codeSent: "Код отправлен",
  emailVerified: "Эл. почта подтверждена",
  phoneVerified: "Телефон подтверждён",
  codeSendFailed: "Не удалось отправить код подтверждения",
  verifyFailed: "Неверный или истёкший код",
  verifyBeforeSave: "Подтвердите новую эл. почту или номер телефона перед сохранением",
  emailVerifyTitle: "Подтвердите вашу новую эл. почту",
  phoneVerifyTitle: "Подтвердите ваш новый номер телефона",
  waitingForConfirmation: "Ожидание подтверждения…",
  invalidPhone: "Введите корректный 10-значный номер телефона",
  invalidEmail: "Введите корректный адрес эл. почты (напр. you@example.com)",

  // Field validation
  errBusinessNameRequired: "Название бизнеса обязательно",
  errEmailRequired: "Адрес электронной почты обязателен",
  errEmailInvalid: "Введите корректный адрес эл. почты (напр. you@example.com)",
  errCategoryRequired: "Выберите категорию бизнеса",
  errCountryRequired: "Выберите страну",
  errAddressLine1Required: "Адрес, строка 1, обязателен",
  errCityRequired: "Город обязателен",
  errRegionRequired: "Штат / область / регион обязателен",
  errPostalCodeRequired: "Почтовый индекс обязателен",

  // Save
  saved: "Сохранено",
  saveChanges: "Сохранить изменения",
  saveFailed: "Не удалось сохранить изменения",
}

export default settings
