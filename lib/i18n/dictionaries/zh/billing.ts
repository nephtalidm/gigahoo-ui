const billing = {
  // Page header
  title: "套餐与计费",
  description: "管理您的订阅、用量和付款详情。",

  // Usage summary
  currentPlan: "当前套餐",
  minutesPerMonth: "{minutes} 分钟/月",
  billingCycle: "计费周期：{period}",
  minutesUsed: "已用分钟数",
  includedMinutes: "套餐内分钟数",
  remaining: "剩余",
  usedThisCycle: "本周期已使用套餐内分钟数的 {percent}%",

  // Plan selection
  choosePlan: "选择您的套餐",
  currentPlanBadge: "当前套餐",
  perMonth: "/月",
  upgradeTo: "升级到 {plan}",
  switchTo: "切换到 {plan}",
  downgradeTo: "降级到 {plan}",
  downgradeScheduledTitle: "已安排降级",
  downgradeScheduledText: "在当前套餐续费前你可继续使用它，之后将切换到较低套餐。",
  downgradePendingBanner: "你将于 {date} 降级到 {plan}。在此之前你可继续使用当前套餐。",
  cancelDowngrade: "取消降级",
  confirmCancelDowngradeTitle: "取消已安排的降级？",
  confirmCancelDowngradeText: "您的套餐将于{date}变更为{plan}。取消后，您当前的套餐将继续保持不变。",
  confirmCancelDowngradeConfirm: "是，保留我当前的套餐",
  noPlans: "暂无可用套餐。",

  // Payment / Stripe

  // Billing history
  billingHistory: "计费记录",
  noInvoices: "暂无发票。",
  downloadInvoice: "下载发票 {number}",

  // Toasts
  changePlanFailed: "套餐更改失败",
  checkoutFailed: "结账启动失败",
  tryAgain: "请重试。",
  // Payment methods (embedded Stripe Elements)
  paymentMethodsTitle: "支付方式",
  paymentMethodsDescription: "添加并管理用于支付订阅的银行卡。",
  addPaymentMethod: "添加支付方式",
  addPaymentMethodHint: "安全地添加新卡。您的信息由我们的支付服务商处理。",
  save: "保存卡片",
  cancel: "取消",
  remove: "移除",
  defaultCard: "默认",
  makeDefault: "设为默认",
  removeConfirm: "移除此支付方式？",
  noPaymentMethods: "尚未保存任何支付方式。",
  cardSaved: "卡片已保存",
  cardExpires: "有效期至 {date}",
  cardError: "无法保存您的卡片。请重试。",
  pay: "支付",
  completeUpgradeTo: "完成升级到{plan}套餐",
  completeSubscribeTo: "完成订阅{plan}套餐",
  confirmSubscribeTitle: "确认订阅",
  confirmSubscribeText: "以每月{price}订阅{plan}？将从尾号{last4}的{brand}卡扣款。",
  confirmUpgradeTitle: "确认升级",
  confirmUpgradeText: "以每月{price}升级到{plan}？将从尾号{last4}的{brand}卡扣款。",
  confirmCharge: "确认并支付",
  addCardFailed: "无法开始添加卡片。请重试。",
  providerUnsupported: "尚不支持此支付服务商。",
}

export default billing
